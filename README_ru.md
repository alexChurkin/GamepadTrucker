...
- опционально: **подсветка DualSense показывает обороты** (зелёный → жёлтый → красный)

## Подсветка DualSense = обороты двигателя (опционально)

С установленным плагином телеметрии SCS подсветка DualSense горит зелёным на
низких оборотах, жёлтым в средней зоне и красным у отсечки (белым — пока ждёт
игру). Настройка один раз:
